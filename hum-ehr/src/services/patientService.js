...
export const fetchActivePatients = async (params = {}) => {
	const payload = buildActivePatientListRequest(params);
	const response = await apiPost(ENDPOINTS.patient.activeList, payload);

	return {
		...response,
		rows: response?.status === 'success' ? (response.data || []).map(mapActivePatientRow) : [],
		totalRecords: response?.recordsFiltered || 0,
		request: payload,
	};
};

export const fetchPatientDetails = (patientId) => apiPostForm(ENDPOINTS.patient.details, { patientId });

export const downloadPatientCCD = (patientId) => apiPost(ENDPOINTS.patient.ccdDownload, { patientId }, { responseType: 'blob' });
