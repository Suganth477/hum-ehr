// LEGACY HOOK — kept for reference during migration.
// This big grab-bag hook is largely SUPERSEDED by the services/ layer
// (apiClient, patientService, allergyService, lookupService) and the helpers in
// utils/commonUtility.js. Prefer those in new code; delete this once every
// screen has stopped importing it.
// useUtilities.js
...
const useUtilities = () => {
    ...
    const getListOfFacilities = useCallback((physicianId = '', clinicianId = '', patientId = '', careGroupId = '') => {
        return axios.post(`${baseUrl}/facilities`, { physicianId, clinicianId, patientId, careGroupId }, { headers: { 'x-auth-token': token } })
            .then(response => {
            if (response.data.status === "success") {
                return response.data.data || {};
            }
