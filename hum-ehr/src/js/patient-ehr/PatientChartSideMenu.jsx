import { useEffect } from 'react';
import {
    LegacyIcon,
    AllergyIcon,
    MedicationIcon,
    ImmunizationIcon,
    DashboardIcon,
    SickFaceIcon,
    CounterIcon,
    HeartCogIcon,
    BedPulseIcon,
    HospitalIcon,
    DevicesIcon,
    HeartPulseIcon,
    ClipboardPlayIcon,
    FilmstripIcon,
    NoteCheckIcon,
    FileDocumentsIcon,
    AccountArrowIcon,
} from '../../components/common/CustomIcons';
const PatientChartSideMenu = ({ patientId, activeSection, onSectionChange }) => {
    useEffect(() => {
        const cached = sessionStorage.getItem('patientChartInformation');
        if (!cached)
            return;
        try {
            const parsed = JSON.parse(cached);
            const savedCode = parsed[`${patientId}_patient_details`]?.selectedMenuCode;
            if (savedCode && savedCode !== activeSection)
                onSectionChange(savedCode);
        }
        catch (error) {
            console.error('Failed to read saved chart menu.', error);
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [patientId]);
    const handleMenuTabClick = (sectionCode) => {
        onSectionChange(sectionCode);
        const cached = sessionStorage.getItem('patientChartInformation');
        if (!cached)
            return;
        try {
            const parsed = JSON.parse(cached);
            const block = parsed[`${patientId}_patient_details`];
            if (block) {
                block.selectedMenuCode = sectionCode;
                sessionStorage.setItem('patientChartInformation', JSON.stringify(parsed));
            }
        }
        catch (error) {
            console.error('Failed to persist chart menu.', error);
        }
    };
    // `iconClass` renders the original MDI glyph via LegacyIcon; `IconComp`
    // renders a custom SVG icon that reproduces the original FA/MDI glyph.
    // Exactly one is supplied per item.
    const renderMenuItem = (code, iconClass, label, IconComp = null, customSvg = null) => {
        const isActive = activeSection === code;
        return (<li className={`pcm-patient-chart-menu-list-item nav-item w-100 ${isActive ? 'active' : ''}`} role="presentation">
        <button className={`nav-link pcm-nav-link text-center w-100 d-flex flex-column align-items-center ${isActive ? 'active' : ''}`} type="button" onClick={() => handleMenuTabClick(code)}>
          {customSvg ?? (IconComp
            ? <span className="pcm-patient-chart-menu-icon"><IconComp/></span>
            : <LegacyIcon icon={iconClass} className="pcm-patient-chart-menu-icon"/>)}
          <span className="pcm-patient-chart-menu-desc text-center text-wrap mt-1">{label}</span>
        </button>
      </li>);
    };
    return (<div className="patient-side-menu-container-node">
      <ul className="pcm-patient-chart-menu-list custom-scrollbar d-flex flex-column" role="tablist">
        {renderMenuItem('PCSUM', '', 'Summary', DashboardIcon)}
        {renderMenuItem('PCPP', 'mdi-id-card', 'Patient Profile')}
        {renderMenuItem('PCAPP', 'mdi-calendar-account-outline', 'Appointments')}
        {renderMenuItem('PCALL', '', 'Allergies', AllergyIcon)}
        {renderMenuItem('PCPRO', '', 'Problems', SickFaceIcon)}
        {renderMenuItem('PCENC', '', 'Encounters', CounterIcon)}
        {renderMenuItem('PCMED', '', 'Medications', MedicationIcon)}
        {renderMenuItem('PCFAH', 'mdi-account-multiple-outline', 'Family Health History')}
        {renderMenuItem('PCPRE', '', 'Preferences', HeartCogIcon)}
        {renderMenuItem('PCPS', '', 'Procedure', BedPulseIcon)}
        {renderMenuItem('PCSUH', '', 'Surgical History', null, (<span className="pcm-patient-chart-menu-icon">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
              <path d="M1.5 3.33437V1.75C1.5 1.33438 1.16562 1 0.75 1C0.334375 1 0 1.33438 0 1.75V5.25C0 5.66563 0.334375 6 0.75 6H4.25C4.66563 6 5 5.66563 5 5.25C5 4.83437 4.66563 4.5 4.25 4.5H2.52187C3.67812 2.69375 5.7 1.5 8 1.5C11.5906 1.5 14.5 4.40938 14.5 8C14.5 11.5906 11.5906 14.5 8 14.5C6.67188 14.5 5.44063 14.1031 4.41563 13.4219C4.07188 13.1938 3.60625 13.2875 3.375 13.6313C3.14375 13.975 3.24062 14.4406 3.58437 14.6719C4.85 15.5125 6.36875 16 8 16C12.4187 16 16 12.4187 16 8C16 3.58125 12.4187 0 8 0C5.32188 0 2.95 1.31562 1.5 3.33437ZM8 4C7.58437 4 7.25 4.33437 7.25 4.75V8C7.25 8.2 7.32812 8.39062 7.46875 8.53125L9.71875 10.7812C10.0125 11.075 10.4875 11.075 10.7781 10.7812C11.0687 10.4875 11.0719 10.0125 10.7781 9.72188L8.74687 7.69063V4.75C8.74687 4.33437 8.4125 4 7.99687 4H8Z" fill="#2E384D"/>
            </svg>
          </span>))}
        {renderMenuItem('PCHPS', '', 'Hospitalization', HospitalIcon)}
        {renderMenuItem('PCIMP', '', 'Implantable Devices', DevicesIcon)}
        {renderMenuItem('PCVIT', '', 'Vitals', HeartPulseIcon)}
        {renderMenuItem('PCIMM', '', 'Immunization', ImmunizationIcon)}
        {renderMenuItem('PCGOAL', 'mdi-bullseye', 'Goals')}
        {renderMenuItem('NUTRIREC', 'mdi-food-apple-outline', 'Nutrition Recommendation')}
        {renderMenuItem('PCCLT', '', 'Clinical Test', ClipboardPlayIcon)}
        {renderMenuItem('PCIMG', '', 'Imaging Orders', FilmstripIcon)}
        {renderMenuItem('PCHSA', '', 'Health Status Assessment', NoteCheckIcon)}
        {renderMenuItem('PCDOC', '', 'Documents', FileDocumentsIcon)}
        {renderMenuItem('PCREF', '', 'Referrals', AccountArrowIcon)}
      </ul>
    </div>);
};
export default PatientChartSideMenu;
